pipeline {
    ...
    environment {
        ...
        EC2_HOST = "13.202.246.246"      // Replace with your EC2 instance IP
    ...
    steps {
        script {
            ...
            sh '''
            scp -o StrictHostKeyChecking=no -i /var/lib/jenkins/jenkins.pem -r app/main.py app/templates ubuntu@13.202.246.246:/home/ubuntu/USER-DETAILS/app/
            '''
            
            echo "Restarting application on EC2..."
            sh '''
            ssh -o StrictHostKeyChecking=no -i /var/lib/jenkins/jenkins.pem ubuntu@13.202.246.246<<EOF
